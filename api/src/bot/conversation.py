...
from .constants import COMMA_DELIMITER
...
def get_crop_msp_history_data(cropname):
    '''Convert a pyplot of last five years MSP data of a crop in base64 and return it
    '''
    if not cropname:
        return None
    
    with open('csv_files/crops.csv', 'r') as msp_file:
        line = msp_file.readline()
        headers = line.split(COMMA_DELIMITER)[1:]
        years = list(map(float, headers))
        line = msp_file.readline()
        while line:
            ...
            if data[0] == cropname:
                ...
                plt.title(f'MSP for {cropname}')
                fig_file = BytesIO()
                plt.savefig(fig_file, format='png')
                fig_file.seek(0)

                fig_data_in_base64 = base64.b64encode(fig_file.getvalue())
                return fig_data_in_base64
            line = msp_file.readline()
